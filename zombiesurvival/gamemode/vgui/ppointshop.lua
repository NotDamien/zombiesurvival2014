...
local function BuyExAmmoDoClick(self)

	RunConsoleCommand("zs_pointsshopbuy",( "ps_"..self.AmmoType.."ex"))
...
local function ItemPanelThink(self)
	...
	if itemtab then
		...
		local wavestate = not GAMEMODE:GetWaveActive()

		local newstate = MySelf:GetPoints() >= math.ceil(itemtab.Worth * (wavestate and GAMEMODE.ArsenalCrateMultiplier or 1)) and not (itemtab.NoClassicMode and GAMEMODE:IsClassicMode())

		if newstate ~= self.m_LastAbleToBuy or wavestate ~= self.m_LastWaveState then

			self.m_LastAbleToBuy = newstate

			self.m_LastWaveState = wavestate

			

			local nameText, pointText

			if wavestate then

				nameText = itemtab.Name.." ("..tostring(math.ceil(100*(1-GAMEMODE.ArsenalCrateMultiplier))).."% off!)"

				pointText = tostring(math.ceil(itemtab.Worth*GAMEMODE.ArsenalCrateMultiplier)).." points"

			else

				nameText = itemtab.Name

				pointText = tostring(itemtab.Worth).." points"

			end

			

			self.m_NameLabel:SetText(nameText)

			self.m_PriceLabel:SetText(pointText)
...
local function PointsShopThink(self)
	...
	if GAMEMODE:GetWave() ~= self.m_LastWaveWarning and not GAMEMODE:GetWaveActive() and CurTime() >= GAMEMODE:GetWaveStart() - 10 and CurTime() > (self.m_LastWaveWarningTime or 0) + 11 then
		...
		timer.SimpleEx(0.6, surface.PlaySound, "ambient/alarms/klaxon1.wav")

		timer.SimpleEx(1.2, surface.PlaySound, "ambient/alarms/klaxon1.wav")

		timer.SimpleEx(2, surface.PlaySound, "vo/npc/Barney/ba_hurryup.wav")
...
function GM:OpenPointsShop()
	...
	local wid, hei = 800, math.max(ScrH() * 0.75, 620)
		...
		if hasitems then
			...
			for i, tab in ipairs(GAMEMODE.Items) do
				...
				if tab.Category == catid and tab.PointShop then
					...
					local mdl = weptab.WorldModel or tab.Model
					...
					namelab:SetPos(120, itempan:GetTall() * 0.5 - namelab:GetTall() * 0.5)
					...
					if weptab and weptab.Primary then
						...
						if ammonames[ammotype] then
							...
							local exammobutton = vgui.Create("DImageButton", itempan)

							exammobutton:SetImage("icon16/box.png")

							exammobutton:SizeToContents()

							exammobutton:CopyPos(button)

							exammobutton:MoveLeftOf(button, 4)

							exammobutton:SetTooltip("Purchase 3 boxes of ammunition")

							exammobutton.AmmoType = ammonames[ammotype]

							exammobutton.DoClick = BuyExAmmoDoClick
							...
							ammobutton:CopyPos(exammobutton)

							ammobutton:MoveLeftOf(exammobutton, 4)

							ammobutton:SetTooltip("Purchase a box ammunition")
